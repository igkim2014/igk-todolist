...
const AdminHolidayPage = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setErrors({ api: err.response?.data?.error?.message || '국경일 수정에 실패했습니다.' });
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('정말로 이 국경일을 삭제하시겠습니까?')) {
      try {
        ...
        setErrors({ api: err.response?.data?.error?.message || '국경일 삭제에 실패했습니다.' });
      }
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setErrors({ api: err.response?.data?.error?.message || '국경일 추가에 실패했습니다.' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">국경일 관리</h1>
        <p className="text-gray-600 dark:text-gray-400">국경일 정보를 관리하세요 (관리자 전용)</p>
      </div>
      ...
      {showAddForm && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">새 국경일 추가</h2>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">등록된 국경일이 없습니다</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              {holidaysData.map((holiday) => (
                <tr key={holiday.holidayId} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  {editingId === holiday.holidayId ? (
                      ...
                      <form onSubmit={handleUpdate} className="space-y-4">
                        ...
                        <div className="flex items-center">
                          <input
                            ...
                            className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                          />
                          <label htmlFor={`editIsRecurring-${holiday.holidayId}`} className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                            매년 반복
                          </label>
                        </div>
                    ...
                    <>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500 dark:text-gray-400">{formatDateKorean(holiday.date)}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate">{holiday.description}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
