...
- `schema.sql` 실행 (`psql -U postgres -d igk_todolist_dev -f schema.sql`)
...
- [x] 7개 디렉토리 생성
- [x] 기본 파일 생성 (`app.js`, `server.js`)
- [x] 디렉토리 구조가 설계 원칙과 일치

**의존성**:

- [x] Task 2.1 완료 (프로젝트 초기화)
...
- [x] `database.js` 작성 완료
- [x] Connection Pool 설정 (max: 10)
- [x] 연결 테스트 성공
- [x] 에러 로그 출력 확인

**의존성**:

- [x] Task 2.2 완료 (디렉토리 구조)
- [x] Task 1.3 완료 (DB 준비)
...
- [x] 회원가입 API 동작 확인 (이메일 중복 체크)
- [x] 로그인 API 동작 확인 (Access + Refresh Token 발급)
- [x] 토큰 갱신 API 동작 확인
- [x] 비밀번호 bcrypt 해싱 확인
- [x] 에러 응답 확인 (400, 401, 409)

**의존성**:

- [x] Task 2.3 완료 (DB 연결)
- [x] Task 2.4 완료 (JWT 유틸리티)
- [x] Task 2.5 완료 (비밀번호 해싱)
...
- [x] 7개 API 엔드포인트 동작 확인
- [x] 인증 미들웨어 적용
- [x] 권한 체크 (타인의 할일 접근 금지)
- [x] 소프트 삭제 동작 확인 (status='deleted', deletedAt 기록)
- [x] 날짜 검증 동작 확인
- [x] 에러 응답 확인 (400, 403, 404)

**의존성**:

- [x] Task 2.3 완료 (DB 연결)
- [x] Task 2.6 완료 (인증 미들웨어)
...
- [x] 휴지통 조회 API 동작 확인
- [x] 영구 삭제 API 동작 확인 (DB에서 완전히 제거)
- [x] 권한 체크 동작 확인
- [x] 에러 응답 확인 (404, 400)

**의존성**:

- [x] Task 2.9 완료 (할일 API)
...
- [x] 국경일 조회 API 동작 확인 (인증 필요)
- [x] 국경일 추가 API 동작 확인 (관리자만 가능)
- [x] 국경일 수정 API 동작 확인 (관리자만 가능)
- [x] 연도/월 필터링 동작 확인
- [x] 에러 응답 확인 (403, 404)
...
- [x] 4개 라우트 연결 확인
- [x] 에러 핸들러 동작 확인
- [x] 서버 실행 확인 (`http://localhost:3000`)

**의존성**:

- [x] Task 2.8, 2.9, 2.10, 2.11 완료 (모든 라우트)
...
- [x] Axios 인스턴스 설정 완료
- [x] 인터셉터 동작 확인
- [x] 환경 변수 (`VITE_API_BASE_URL`) 사용 확인

**의존성**:

- [x] Task 3.2 완료 (디렉토리 구조)
...
- [x] Task 3.3 완료 (Axios 인스턴스)
- [x] Task 3.4 완료 (tokenManager)
...
- [x] 6개 라우트 정의 완료
- [x] Protected Route 동작 확인
- [x] 인증 체크 동작 확인
- [x] 리다이렉트 동작 확인

**의존성**:

- [x] Task 3.5 완료 (authStore)
...
- [x] Task 3.7 완료 (todoStore)
- [x] Task 3.12 완료 (할일 컴포넌트)
...
- [x] Task 3.7 완료 (todoStore, uiStore)
- [x] Task 3.8 완료 (Modal 컴포넌트)
...
- [x] 모바일 화면에서 사용 가능
- [x] 터치 UI 최적화 확인
- [x] 크로스 브라우저 테스트 (Chrome, Safari)

**의존성**:

- [x] Task 3.13, 3.15, 3.16, 3.17 완료 (모든 페이지)
